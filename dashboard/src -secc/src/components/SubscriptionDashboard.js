...
const SubscriptionDashboard = () => {
  ...
  const fetchDataAndCreateCharts = () => {
    ...
    if (barChartRef.current) {
      fetchChartData(`http://localhost:5000/subscription_dashboard/barchart?product=${product}&state=${state}`, createBarChart);
    }

    if (lineChartRef.current) {
      fetchChartData(`http://localhost:5000/subscription_dashboard/linechart?product=${product}&state=${state}`, createLineChart);
    }
  };
